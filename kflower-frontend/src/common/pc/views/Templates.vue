      ...
      <div class="import-container">
        ...
        <div v-if="importStep === 0">
          ...
          <el-upload
            ...
            drag
            :limit="1"
            accept=".xlsx,.xls,.csv,.docx,.json,.png,.jpg,.jpeg,.bmp"
            :auto-upload="false"
          ...
          >
            ...
            <div class="upload-text">
              ...
              <p class="upload-hint">支持 Excel (.xlsx/.xls/.csv)、Word (.docx)、图片 (.png/.jpg/.jpeg)</p>
            </div>
          </el-upload>
        ...
        <div v-if="importStep === 1">
          <!-- 表头行选择提示 -->
          <el-alert 
            v-if="importData.potential_headers && importData.potential_headers.length > 1"
            :title="`已识别 ${importData.all_rows.length} 行数据，请选择包含表头的行`" 
            type="info" 
            show-icon 
            :closable="false"
            style="margin-bottom:12px"
          />
          <el-alert v-else :title="`已识别 ${importData.total_rows} 行 × ${importData.total_columns} 列`" type="success" show-icon style="margin-bottom:12px" />
          
          <div class="preview-controls">
            <!-- 表头行选择 -->
            <div v-if="importData.potential_headers && importData.potential_headers.length > 1" class="header-row-selector">
              <div class="selector-title"><el-icon><List /></el-icon> 选择表头行（点击预览该行数据）:</div>
              <div class="header-row-list">
                <div 
                  v-for="ph in importData.potential_headers.slice(0, 10)" 
                  :key="ph.row"
                  class="header-row-item"
                  :class="{ active: importHeaderRow === ph.row, recommended: ph.is_potential && ph.confidence > 0.6 }"
                  @click="selectHeaderRow(ph.row)"
                >
                  <div class="row-badge">
                    <el-tag v-if="ph.is_potential && ph.confidence > 0.6" type="success" size="small">推荐</el-tag>
                    <span class="row-num">第{{ ph.row + 1 }}行</span>
                    <span class="row-confidence" v-if="ph.confidence > 0">置信度: {{ (ph.confidence * 100).toFixed(0) }}%</span>
                  </div>
                  <div class="row-preview">{{ ph.preview || '(空行)' }}</div>
                </div>
              </div>
            </div>
            
            <!-- 工作表选择 -->
            <div v-if="importData.sheet_names && importData.sheet_names.length > 1" class="control-row">
              <span>工作表：</span>
              <el-select v-model="importSheetName" placeholder="选择工作表" style="width:160px" @change="reparseWithSheet">
          ...
          <el-table :data="importData.rows.slice(0, 20)" border size="small" max-height="300" style="margin-top:12px">
...
async function confirmCreateOrUpdate() {
  ...
  try {
    if (editingTemplate.value) {
      ...
      const res: any = await templateAPI.create({ name:editForm.name, code:editForm.code, description:editForm.description, category:editForm.category, is_active:editForm.is_active, modules:[] })
...
const importData = reactive({ 
  ...
  header_row: 0,
  potential_headers: [] as any[],  // 候选表头行列表
...
async function doParseFile(rawFile: File, headerRow: number, sheetName: string) {
  ...
  try {
    ...
    if (json.success) {
      ...
      const errMsg = json.detail || json.message || '解析失败'
      ElMessage.error({ message: errMsg, duration: 0, showClose: true })
    }
  } catch (e: any) {
    ElMessage.error({ message: '解析失败: ' + (e.message || '请检查文件格式'), duration: 0, showClose: true })
  }
}
  ...
  {"type":"textarea","label":"备注","name":"remark","required":false,"width":"100%","placeholder":"请输入备注"}
    ...
    const response = await fetch('/api/v1/agent/chat', {
      ...
      body: JSON.stringify({
        ...
        enable_tools: false,
        model: selectedModelId.value || undefined,  // 传递选中的模型
        provider: selectedModel?.provider || undefined  // 传递选中的提供商
      })
    })
...
.header-row-selector .selector-title {
  font-size: 14px;
  color: #67c23a;
  margin-bottom: 10px;
  display: flex;
...
.header-row-item.active {
  border-color: #67c23a;
  background: #f0f9eb;
}
.header-row-item.recommended {
  border-color: #67c23a;
}
.header-row-item .row-badge {
  display: flex;
  align-items: center;
  gap: 6px;
  margin-bottom: 4px;
}
.header-row-item .row-num {
  font-size: 12px;
  color: #909399;
  font-weight: 500;
}
.header-row-item .row-confidence {
  font-size: 11px;
  color: #c0c4cc;
}
.header-row-item .row-preview {
  font-size: 12px;
  color: #606266;
  overflow: hidden;
