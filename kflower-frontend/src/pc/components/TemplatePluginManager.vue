<template>
  <div class="template-plugin-manager">
    <!-- Header -->
    ...
    <!-- Bound Plugins List -->
    <div v-if="loading" class="tpm-loading">
      <el-icon class="is-loading"><Loading /></el-icon> 加载中...
    </div>
    <div v-else-if="boundPlugins.length === 0" class="tpm-empty">
      <el-empty description="暂无绑定的插件" :image-size="60">
        <el-button type="primary" size="small" @click="showBindDialog = true">绑定插件</el-button>
      </el-empty>
    </div>
    <div v-else class="tpm-list">
      <div v-for="bp in boundPlugins" :key="bp.id" class="tpm-item">
        <div class="tpm-item-icon">
          <el-icon size="24"><Box /></el-icon>
        </div>
        <div class="tpm-item-info">
          <div class="tpm-item-name">{{ bp.display_name }}</div>
          <div class="tpm-item-meta">
            <el-tag size="small" type="info">{{ bp.version }}</el-tag>
            <span class="tpm-item-hooks">钩子: {{ Object.keys(bp.hook_code || {}).join(', ') || '无' }}</span>
          </div>
        </div>
        <div class="tpm-item-actions">
...
import * as templatePluginApi from '../../common/api/templatePlugin'
...
async function loadBoundPlugins() {
  loading.value = true
  try {
    const res = await templatePluginApi.getTemplatePlugins(props.templateId)
...
async function loadAvailablePlugins() {
  ...
  try {
    const res = await templatePluginApi.getAvailablePlugins(props.templateId, {
...
async function bindPlugin() {
  ...
  binding.value = true
  try {
    await templatePluginApi.bindPluginToTemplate(props.templateId, selectedPlugin.value.id)
...
async function unbindPlugin(plugin: any) {
  try {
    ...
    await templatePluginApi.unbindPlugin(props.templateId, plugin.id)
...
async function updateBinding(plugin: any) {
  try {
    await templatePluginApi.updateTemplatePluginBinding(props.templateId, plugin.id, {
...
async function saveConfig() {
  ...
  try {
    ...
    await templatePluginApi.updateTemplatePluginBinding(props.templateId, configPlugin.value.id, { config })
...
async function runTest() {
  ...
  try {
    ...
    const res = await templatePluginApi.triggerTemplatePluginHook(props.templateId, testHookName.value, testData)
