...
# 魔塔社区模型下载工具
from app.core.ai_digital_base.modelscope_utils import ensure_model_downloaded, setup_modelscope_environment
...
class EmbeddingService:
    ...
    def _load_local_model(self, model_name: str, model_path: str = None):
        ...
        if model_path and os.path.exists(model_path):
            ...
            logger.info(f"尝试从魔塔社区获取模型: {model_name}")
            downloaded_path = ensure_model_downloaded(model_name, cache_dir=cache_dir)
            
            if downloaded_path and os.path.exists(downloaded_path):
                load_path = downloaded_path
                logger.info(f"使用魔塔社区模型: {load_path}")
            else:
                raise FileNotFoundError(
                    f"无法获取模型: {model_name}\n"
                    f"从魔塔社区下载失败，请检查网络连接或手动下载模型。"
                )
        else:
            raise FileNotFoundError(
                f"未配置本地 embedding 模型路径 (model_path) 或模型名称。\n"
